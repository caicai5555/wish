...
function changeParent() {
  var interval = setInterval(function() {
    ...
    if (iframes.length > 0) {
      iframes[0].contentWindow.parent = top.__activityModalWindow || top;
      clearInterval(interval);
    }
  }, 500);
}

function fixSearchBug(){
	var searchButton;
	if((searchButton=$('button:contains("搜索")'),searchButton.length !== 0) || (searchButton=$('input[value="搜索"]'),searchButton.length !== 0)){
		var $form = searchButton.parentsUntil('form').last().parent();
		var $pageNo = $form.find('input[name=pageNo]');
		$form.find('input[type="text"]').keydown(function(event){
			if(event.keyCode === 13){
				$pageNo.val(1);
			}
		});
...
! function(a, b) {
  ...
  a.layer = {
    ...
    config: function(a, b) {
      var d = 0;
      return a = a || {}, layer.cache = e.config = c.extend(e.config, a), layer.path = e.config.path || layer.path, "string" == typeof a.extend && (a.extend = [a.extend]), layer.use("../jingle/css/layer/skin/layer.css", a.extend && a.extend.length > 0 ? function f() {
        var c = a.extend;
  ...
  g.anim = ["layui-anim", "layui-anim-01", "layui-anim-02", "layui-anim-03", "layui-anim-04", "layui-anim-05", "layui-anim-06"], f.pt.config = {
    ...
    switch ("string" == typeof b.area && (b.area = "auto" === b.area ? ["", ""] : [b.area, ""]), b.type) {
      ...
      case 2:
        ...
        b.content = '<iframe scrolling="' + (b.content[1] || "auto") + '" allowtransparency="true" id="' + g[4] + f + '" name="' + g[4] + f + '" onload="this.className=\'\';top.changeParent()" class="layui-layer-load" frameborder="0" src="' + b.content[0] + '"></iframe>';
        break;
      case 3:
    ...
    if(window === top && window !== parent){
      window.__activityModalWindow.isUseTopLayer = false;
    }
    var b = c("#" + g[0] + a),
      d = b.attr("type");
    if (b[0]) {
    ...
    //去除遮罩层和进度条
    if (a = Number(a), typeof a === 'number') {
      var n = a;
//      while (n--) {
//        var _arr = [];
//        $('[times=' + n + ']').each(function(i, e) {
//          var $this = $(this);
//          _arr.push($this.hasClass('layui-layer-shade') || $this.hasClass('layui-layer-loading'));
//        });
//        var _isAll = /^(true,?)+$/.test(_arr.join(','));
//        if (_isAll) {
//          $('[times=' + n + ']').remove();
//        }
//      }
//      //去除所有前边的弹窗
      ...
      $('[times]').each(function(i, e) {
        ...
        indexArr.push({index:index, element: $this});
      });
      indexArr.sort(function(a,b){
    	  return a.index < b.index;
      });
      while(indexArr.length > 0){
      var maxIndexElement = indexArr[0].element;
      if(maxIndexElement.hasClass('layui-layer-shade') || maxIndexElement.hasClass('layui-layer-loading')){
    	  maxIndexElement.remove();
      }
      indexArr.shift();
      }
    }
    ...
    if(!window.isInTopWindow){
      if(window.isInSecondIframe && typeof a.content === 'string' && /(\/\w+(?=\/|$))+/.exec(a.content)){
        window.isUseTopLayer = true;
        top.__activityModalWindow = window;
        top.layer.open(a);
        return;
      }
    	}
    ...
    e.run(), layer.use("../jingle/css/layer/skin/layer.css")
  }()
}(window);

var topLayer = top.layer;
var selfLayer = self.layer;
var isInTopWindow = top === self && top === parent;
function changeLayer() {
  window.isInSecondIframe = true;
  window.isUseTopLayer = false;
  Object.defineProperty(window, 'layer', {
    get: function() {
      if (window.isUseTopLayer) {
        return topLayer;
      } else {
        return selfLayer;
      }
...
$(function(){
	$('.btn[value=取消]').add($('.btn:contains(取消)')).click(function(e){
