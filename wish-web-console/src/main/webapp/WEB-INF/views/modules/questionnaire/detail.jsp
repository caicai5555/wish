...
<form id="inputForm" action="/admin/questionnaire/update"  method="post" class="form-horizontal">
  ...
  <div class="control-group">
    ...
    <div class="controls">
      <span class="form-control"> ${entity.repstQuestionnaire.questionnaireName}</span>
    </div>
  </div>

  <c:forEach items="${entity.repstQuestionnaire.repstQuestionsCtgrList}" var="element" varStatus="status">
    <div class="control-group">
        ...
        <div class="control-group">
          ...
          <c:forEach items="${question.repstQuestionItemList}" var="item" varStatus="status">
            <label class="checkbox inline">
              <input type="radio" value="${item.id}" name="repstQuestionItemList[${questionStatus.count}].itemId" <c:if test="${item.repstUserQuestion != null}"> checked="checked"</c:if>> ${item.itemName}
            </label>
          </c:forEach>
        </div>
      </c:if>
