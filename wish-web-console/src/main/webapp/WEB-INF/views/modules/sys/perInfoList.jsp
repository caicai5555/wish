...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tr>
			<th>姓名</th>
			<th>身份证</th>
			<th class="sort-column login_name">邮箱</th>
			<th class="sort-column name">电话</th>
			<th>手机</th>
			<th>住址</th>
			<th>建档年龄</th>
			<th>民族</th>
		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				<td>${user.name}</td>
				<td>${user.cardNo}</td>
				<td><a href="${ctx}/sys/user/form?id=${user.id}">${user.email}</a></td>
				<td>${user.phone}</td>
				<td>${user.mobile}</td>
				<td>${user.address}</td>
				<td>111</td>
				...
				<shiro:hasPermission name="sys:user:edit"><td>
    				<a href="${ctx}/sys/user/perInfoForm?id=${user.id}">修改</a>
					<%--<a href="${ctx}/sys/user/delete?id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)">查看</a>--%>
					<a href="${ctx}/archive/indicatorsPage?userId=${user.id}">指标</a>
				</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
