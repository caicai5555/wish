...
import com.foundation.dao.modules.write.subsystem.SubsystemPlatformUserDao;
...
public class SystemServiceImpl extends BaseServiceImpl implements InitializingBean,ISystemService {
	...
	@Autowired(required = false)
	private SubsystemPlatformUserDao subsystemPlatformUserDao;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public User getByIdOrLoginName(Map<String, Object> params) {
		params.put("DBKEY", new User().getDBKEY());	//DB加密
		return userDaoR.getByIdOrLoginName(params);
	}

	@Override
	public Page<User> findUser(Map<String, Object> params, Page<User> page) {
		params.put("DBKEY", new User().getDBKEY());	//DB加密
		List<User> list = userDaoR.queryPage(params, page);
		page.setList(list);
		return page;
	}
	...
	public void saveUser(User user, String systemCode) {
		saveUser(user);
		SubsystemPlatformUser subsystemPlatformUser = new SubsystemPlatformUser();
		subsystemPlatformUser.setSid(systemCode);
		subsystemPlatformUser.setUid(user.getId());
		subsystemPlatformUserDao.save(subsystemPlatformUser);
	}
}
