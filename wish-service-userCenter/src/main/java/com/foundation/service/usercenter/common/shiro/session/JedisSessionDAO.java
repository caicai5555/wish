package com.foundation.service.usercenter.common.shiro.session;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
    ...
    public void update(Session session) throws UnknownSessionException {
        ...
        try {
            ...
            template.hset(sessionKeyPrefix, session.getId().toString(), principalId + "|" + session.getTimeout() + "|" + session.getLastAccessTime().getTime());
            //template.set(getBytesKey(sessionKeyPrefix + session.getId()), toBytes(session));
            template.set(SerializeUtil.getBytesKey(sessionKeyPrefix + session.getId()), SerializeUtil.toBytes(session));
            // 设置超期时间
            int timeoutSeconds = (int) (session.getTimeout() / 1000);
            template.expire((sessionKeyPrefix + session.getId()), timeoutSeconds);
    ...
    public void delete(Session session) {
        ...
        try {
            ...
            template.hdel(SerializeUtil.getBytesKey(sessionKeyPrefix), SerializeUtil.getBytesKey(session.getId().toString()));
            template.del(SerializeUtil.getBytesKey(sessionKeyPrefix + session.getId()));
    ...
    public Collection<Session> getActiveSessions(boolean includeLeave, Object principal, Session filterSession) {
        ...
        try {
            Map<String, String> map = template.hgetAll(sessionKeyPrefix);
                ...
                if (StringUtils.isNotBlank(e.getKey()) && StringUtils.isNotBlank(e.getValue())) {
                    ...
                    if (ss != null && ss.length == 3) {// jedis.exists(sessionKeyPrefix + e.getKey())){
                        ...
                        // SESSION验证失败
                            ...
                            template.hdel(sessionKeyPrefix, e.getKey());
                        }
                    }
                    // 存储的SESSION不符合规则
                    else {
...
//			if (jedis.exists(sessionKeyPrefix + sessionId)){
            session = (Session) SerializeUtil.toObject(template.get(SerializeUtil.getBytesKey(sessionKeyPrefix + sessionId)));
//			}
