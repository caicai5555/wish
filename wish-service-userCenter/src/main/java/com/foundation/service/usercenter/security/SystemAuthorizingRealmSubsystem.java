...
import com.foundation.service.usercenter.service.impl.SystemServiceSubsystemImpl;
import com.foundation.service.usercenter.service.impl.SystemServiceSubsystemImpl;
 ...
 */
@Service
public class SystemAuthorizingRealmSubsystem extends AuthorizingRealm {

	private Logger logger = LoggerFactory.getLogger(getClass());

	private SystemServiceSubsystemImpl systemServiceSubsystem;
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		// 校验用户名密码
		String code = "";
		if(StringUtils.isNotBlank(token.getCode())){
			code = token.getCode();
		}else{
			throw new AuthenticationException("msg:非认证系统不能登录, 请认证.");
		}
		User user = getSystemService().getUserByLoginName(token.getUsername(), code);
		if (user != null) {
...
//			}
			...
			return new SimpleAuthenticationInfo(new Principal(user, token.isMobileLogin(), token.getCode()),
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		User user = getSystemService().getUserByLoginName(principal.getLoginName(), code);
		if (user != null) {
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			// TODO: 16-12-13 缓存用户的权限
...
//		}
	 ...
	 * 获取系统业务对象
	 */
	public SystemServiceSubsystemImpl getSystemService() {
		if (systemServiceSubsystem == null){
			systemServiceSubsystem = SpringContextHolder.getBean(SystemServiceSubsystemImpl.class);
		}
		return systemServiceSubsystem;
	}
