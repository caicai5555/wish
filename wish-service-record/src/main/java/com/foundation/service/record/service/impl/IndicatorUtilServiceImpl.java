...
import com.foundation.service.dist.service.impl.DistArchiveConclusionServiceImpl;
import com.foundation.service.dist.service.impl.DistArchiveDescindicatorServiceImpl;
import com.foundation.service.dist.service.impl.DistArchiveIndicatorServiceImpl;
import com.foundation.service.record.service.IIndicatorUtilService;
...
/**
 * @author wangsen
 * @ClassName: IndicatorUtilServiceImpl
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @date 2016/10/27 20:28
 */
@Service
public class IndicatorUtilServiceImpl implements IIndicatorUtilService {
    private Log logger = LogFactory.getLog(IndicatorUtilServiceImpl.class);
    ...
    */
    ...
    public IndicatorType getIndicatorType(String code) throws Exception {
        ...
        if(StringUtils.isNotEmpty(type)){
            logger.info("[DistArchiveUtilServiceImpl.getIndicatorType]:::根据指标code="+code+"从缓存中命中类型为："+IndicatorType.getByType(type));
        ...
        if(indicatorCount>0){
            ...
            logger.info("[DistArchiveUtilServiceImpl.getIndicatorType]:::根据指标code="+code+"获取指标类型为："+IndicatorType.INDICATOR);
        ...
        if(bescindicatorCount>0){
            ...
            logger.info("[DistArchiveUtilServiceImpl.getIndicatorType]:::根据指标code="+code+"获取指标类型为："+IndicatorType.DESCINDICATOR);
        ...
        if(conclusionCount>0){
            ...
            logger.info("[DistArchiveUtilServiceImpl.getIndicatorType]:::根据指标code="+code+"获取指标类型为："+IndicatorType.CONCINDICATOR);
